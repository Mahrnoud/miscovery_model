...
class ModelEvaluator:
    ...
    def evaluate(self, epoch, global_step, train_loss=None):
        """
        Evaluate the model and save metrics - UPDATED to use entire test dataset
        """
        self.model.eval()
        total_loss = 0
        all_preds = []
        ...
        print(f"Processing {len(self.eval_dataloader)} batches with full dataset evaluation...")
        ...
        with torch.no_grad():
            for batch_idx, (src, tgt) in enumerate(self.eval_dataloader):
                ...
                try:
                    ...
                    # Generate predictions for ALL samples in the current batch
                    ...
                    if (batch_idx + 1) % 10 == 0:
                        print(
                            f"  Processed batch {batch_idx + 1}/{len(self.eval_dataloader)} - Generated {len(all_preds)} predictions so far")
        ...
        print(f"Evaluation completed. Generated {len(all_preds)} prediction pairs from entire test dataset.")
